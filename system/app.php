...
final class Application{
	...
	public static function init(){
		...
		require SYS_CORE_PATH.'/basemodel.php';
		require SYS_CORE_PATH.'/basecontroller.php';
	}
	...
	public static function routeToCm($url_array = array()){
		...
		if (isset($url_array['controller'])){
			...
			// 如果是app
			$controller_file = ($app) ? CONTROLLER_PATH.'/'.$app.'/'.$controller.'Controller.php' 
					: CONTROLLER_PATH.'/'.$controller.'Controller.php';
			$model_file = ($app) ? MODEL_PATH.'/'.$app.'/'.$model.'Model.php' 
					: MODEL_PATH.'/'.$model.'Model.php';
		} else {
			$controller = $model = self::$_config['route']['default_controller'];
			
			$controller_file = ($app) ? CONTROLLER_PATH.'/'.$app.'/'.self::$_config['route']['default_controller'].'Controller.php'
					: CONTROLLER_PATH.'/'.self::$_config['route']['default_controller'].'Controller.php';
			$model_file = ($app) ? MODEL_PATH.'/'.$app.'/'.self::$_config['route']['default_controller'].'Model.php'
					: MODEL_PATH.'/'.self::$_config['route']['default_controller'].'Model.php';
		}
		
		$action = isset($url_array['action']) ? $url_array['action'] 
		        : self::$_config['route']['default_action'];
